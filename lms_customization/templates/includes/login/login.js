...
window.verify = {};
// Get all elements with class 'form-signin' or 'form-signup'
const forms = document.getElementsByClassName('form-signin form-signup');
for (let form of forms) {
    // Get all elements with class 'page-card-body' within each form
    const body = form.getElementsByClassName('page-card-body');
    for (let b of body) {
        // Create a new div element
        const captchaContainer = document.createElement('div');
        captchaContainer.classList.add('form-group');
        captchaContainer.innerHTML = `
            <img src="" id="captcha_image1" alt="CAPTCHA">
            <button type="button" onclick="reload_captcha1()" class="btn btn-light" style="display: inline-block;">
                <i style="font-size:15px" class="fa fa-refresh"></i> <!-- FontAwesome refresh icon -->
            </button>
            <div class="form-group">
                <input type="text" name="captcha" id="captcha1" class="form-control" placeholder="Enter CAPTCHA" required>
            </div>
        `;
        // Append the new div to the current body element
        b.appendChild(captchaContainer);
    }
}
function reload_captcha1() {
    fetch("/api/method/lms_customization.lms_customization.customization.login.captcha.get_captcha")
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
			console.log(data)
            const captchaText = data.message.captcha_text;
            console.log("CAPTCHA Text:", captchaText);
            
            // Store CAPTCHA text in local storage
            localStorage.setItem('captcha_text1', captchaText);
            
            // Reload the CAPTCHA image by changing its source attribute
			const captchaImage = document.getElementById('captcha_image1');
			captchaImage.src = 'data:image/png;base64,' + data.message.captcha_image;
			console.log('Image src set to:', captchaImage.src);
        })
        .catch(error => {
...
login.call = function (args, callback, url="/") {
	if ((args['cmd'].split('.')).includes('sign_up') && args['captcha'] !== localStorage.getItem('captcha_text1')){
		frappe.throw(__("Invalid CAPTCHA. Please try again."))
	}else if ((args['cmd'].split('.')).includes('login') && args['captcha'] !== localStorage.getItem('captcha_text')){
		frappe.throw(__("Invalid CAPTCHA. Please try again."))
	}
